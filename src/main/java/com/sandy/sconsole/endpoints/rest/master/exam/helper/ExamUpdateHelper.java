...
public class ExamUpdateHelper {
    ...
    private void updateExamState( Exam exam, ExamVO examVO ) {
        boolean examConfigurationComplete = isExamConfigurationComplete( examVO ) ;
        if( !examConfigurationComplete ) {
            exam.setState( "DRAFT" ) ;
        }
        else {
            exam.setState( examVO.getState() ) ;
        }
    }
    
    private boolean isExamConfigurationComplete( ExamVO examVO ) {
        for( ExamSectionVO sectionVO : examVO.getSections() ) {
            if( sectionVO.getQuestions().size() != sectionVO.getNumQuestions() ) {
                return false ;
            }
        }
        return true ;
