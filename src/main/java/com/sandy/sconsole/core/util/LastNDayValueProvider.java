...
public abstract class LastNDayValueProvider
    ...
    public void fullRefresh() {
        ...
        getPastDayValues( this.numPastDays ).forEach( ( _date, value ) -> {
            ...
            if( dayValueMap.containsKey( date ) ) {
                dayValueMap.put( date, new DayValue( date, (float)value/3600 ) ) ;
            }
        } ) ;
    }
    
    public void clearState() { dayValueMap.clear() ;}
    ...
    public final void updateTodayValue() {
        dayValueMap.put( today, new DayValue( today, (float)getTodayValue()/3600 ) ) ;
    }
    
    protected abstract Map<Date, Integer> getPastDayValues( int numPastDays ) ;
    
    protected abstract int getTodayValue() ;
}
